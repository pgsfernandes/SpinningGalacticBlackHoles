function LightRing(f::Field, g::Field, h::Field, W::Field,rh::Double64,q::Double64=0.0; guess = nothing, compareMBH=false)
    y=pi/2

    #Compare with ADM mass or BH mass
    if !compareMBH
        M=GetMass(f,g,h,W,rh)
        j=GetJ(f,g,h,W,rh)
        chi = j/M^2
    else
        Mt, J, χ, Th, Ah, Ωh = get_quantities(f,g,h,W,rh)
    ...
    if compare_kerr
        ...
        RKerr_counter = rhK*(3.0-xkerr_minus)^2/(2*(1-xkerr_minus))*sqrt(gKerrN(xkerr_minus,y,rhK,chi,q)/fKerrN(xkerr_minus,y,rhK,chi,q))
        RKerr_co = rhK*(3.0-xkerr_plus)^2/(2*(1-xkerr_plus))*sqrt(gKerrN(xkerr_plus,y,rhK,chi,q)/fKerrN(xkerr_plus,y,rhK,chi,q))
    end
    ...
    #returns a vector with [R_plus/RKerr_plus-1, w_plus/wKerr_plus-1, R_minus/RKerr_minus-1, w_minus/wKerr_minus-1, sol_plus, sol_minus] when there are two light rings, one co and other counter rotating
